...
install.packages('gridExtra'); install.packages('ggvegan')
library('backports'); library('stringi'); library('vegan'); library('tidyverse'); library('ggplot2')
library('lmerTest'); library('viridis'); library('lme4'); library('dplyr'); library('gridExtra');library(ggvegan)
...
test<-test%>%
  filter(Pair.Paper.2 %in% c(1:14)) ## Filter 14 or 17? (missing three data points)
...
xy<-summary(aov(inundation_days))
hist(residuals(inundation_days))



# RDM, Pool Size and Catchment Size  ----------------------------------------------------------------
qdata<-read.csv("2017-2018 Vegetation Quadrats.csv",fileEncoding="UTF-8-BOM")
qdata_select<- qdata %>% 
  filter(Pair %in% c(1:12))   %>% 
  group_by(Pool.ID, Grazing, SoilType)%>%
...
#Average 2018 Quadrats
#Relate the structure of the vernal pool plant community to grazing and non-grazing realted environmental variables
#Explanatory variables include: Size, Depth, Catchment Area, Soil Type, Inundation Days, Hoof Print Cover, Grazing Duration, RDM

# 2018 Averaged Quadrats in Transition Zones ----------------------------------------------------------------
qdata<-read.csv("2017-2018 Vegetation Quadrats.csv",fileEncoding="UTF-8-BOM")
...
#check out pool size difference
#Pool size by grazing
ggplot(data=qdata_select, aes(x=Grazing,y=Size, fill=Grazing))+
  geom_boxplot()+
  labs(title="Pool Size by Grazing", y="Size", x="Grazing")+
...
p4<-ggplot(fmod, aes(x = CCA1, y = CCA2, label=Label)) +
  ...
  geom_text(data = subset(fmod, Score == "biplot")[c(3:4),], 
            aes(x=CCA1,y=CCA2,label=c("Size", "Hoofprints")), size=7, face='bold', colour = "gray24") + xlab("CCA1") + ylab("CCA2")
p4


# Step 2: LMM to look at explanatory variables vs. species diversity and cover ---------------------------------
...
TukeyHSD(aov(rel_cov_nat~Grazing, transition_diversity)) #relative cover of natives not different between new grazed and grazed
...
LM1 <- lmer(`Species Richness` ~ Grazing+(1|Pool.ID), data=transition_diversity) 
LM2 <- lmer(`Species Richness` ~ Size+(1|Pool.ID), data=transition_diversity)
LM3 <- lmer(`Species Richness` ~ Grazing+Size+(1|Pool.ID), data=transition_diversity) #Best one 
AIC(LM1, LM2, LM3)
anova(LM3)


###Linear model: shannon by grazing and inundation
LM1 <- lmer(`Shannon Diversity` ~ Grazing+(1|Pool.ID), data=transition_diversity) #BEST ONE
LM2 <- lmer(`Shannon Diversity` ~ Size+(1|Pool.ID), data=transition_diversity)
LM3 <- lmer(`Shannon Diversity` ~ Grazing+Size+(1|Pool.ID), data=transition_diversity) #Best one 
LM4 <- lmer(`Shannon Diversity` ~ Inundation_days+(1|Pool.ID), data=transition_diversity)
AIC(LM1, LM2, LM3)
anova(LM1)

###Linear model: natives by grazing and inundation
LM1 <- lmer(`Relative Cover of Natives` ~ Grazing+(1|Pool.ID), data=transition_diversity) #BEST ONE
LM2 <- lmer(`Relative Cover of Natives` ~ Size+(1|Pool.ID), data=transition_diversity)
LM3 <- lmer(`Relative Cover of Natives` ~ Grazing+Size+(1|Pool.ID), data=transition_diversity) 
AIC(LM1, LM2, LM3)
anova(LM1)
...
transition_diversity$Inundation_days<-as.numeric(transition_diversity$Inundation_days)

#Hoofprint count species richness
LM <- lmer(`Species Richness` ~ Hoofprint+(1|Pool.ID), data=transition_grazed)
anova(LM)
ggplot(data=transition_diversity, aes(x=Inundation_days,y=, colour=Grazing))+
  scale_color_manual(values=c("maroon", "turquoise", "blue"))+
  geom_point(size=4)+
...
LM <- lmer(`Shannon Diversity` ~ Hoofprint+(1|Pool.ID), data=transition_grazed)
anova(LM)
ggplot(data=transition_diversity, aes(x=Hoofprint,y=shannon, colour=Grazing))+
  scale_color_manual(values=c("maroon", "turquoise", "blue"))+
  geom_point(size=4)+
  stat_smooth(method='lm', se=FALSE, fullrange = TRUE, size=2)+
  labs(title="Species Richness by Hoofprint Count and Grazing Duration", y="Species Richness", x="Hoofprint Count")+
...
LM <- lmer(rel_cov_nat ~ Hoofprint+(1|Pool.ID), data=transition_diversity)
anova(LM)
...
LM <- lmer("Species Richness" ~ Grazing+ RDM+ Inundarion_Days+ Hoofprint + SoilType+ Size+ Depth+ Catchment, data=transition_diversity)
anova(LM)
...
CIT.test<-ctree(spec_rich ~ Grazing+ RDM+ Inundation_days+ Hoofprint + SoilType+ Size+ Depth+Catchment, data=transition_diversity)
plot(CIT.test)

#better to set it to be univariate
#treatment, size and treatment interactions, hjoofprints and treatment
...
#Plot Species richness by  hoofprint count and treatment 
...
linearMod2 <- lm(spec_rich ~ Hoofprint*Grazing, data=transition_diversity_inundation) 
...
ggplot(data=transition_diversity_inundation, aes(x=days,y=shannon, colour=Grazing))+
  ...
  geom_point(mapping=aes(group=cut_width(days, 15)), size=4)+
  stat_smooth(method='lm', se=FALSE, fullrange = TRUE, size=2)+
  labs(title="Shannon Weiner Diversity by Days of Inundation and Grazing Duration", y="Shannon Weiner", x="Days of Inundation")+
  theme(plot.title=element_text(size=30, hjust=.5))+
  theme(axis.title.x =element_text(size=30))+
  theme(axis.title.y =element_text(size=30))+
  theme(legend.title =element_text(size=30))+
...
whats the most variability and is it my treatment

the cca would tell you how grazing clusters community, do people then do clusters of the ccas with environmental variable.names()

Do a PCA to get a clustering of what is most structuring the pools, testing if that differs by grazing, then 

then use the pca to correlate with environmental variables

regression models on the cca values

looking through the literature


1. Clustering just based on grazing-- how do my grazing treatments most differ in composition

2. what environmental variables may drive that grazing effect?
  
  is the relationship to that cca value
if the grazing and all the env factors are in a cca, might be a more 



Assert that it's not just about grazing pressure
When you're looking across those you do see more increased diversity
