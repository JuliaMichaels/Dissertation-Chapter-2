...
library("rpart"); library("party"); library("partykit"); library('lme4'); library('lmerTest')
...
# Calculate Total days of inundation for each year 2016-2018 ---------------------------------------------------------
...
dl_subset_2017<-data_loggers_2017 %>% # 2017 Pull out subset of dates from dataloggers when staff gauges were also taken
  group_by(Date) %>%
  summarise_all(funs(median)) %>% 
...
# 2016 Pull out subset of dates from dataloggers when staff gauges were also taken
...
#average quadrats to run anovas for three measures of diversity
transition_averaged<-transition_diversity %>% 
  group_by(Pool.ID, Grazing) %>% 
  dplyr::summarize_if(is.numeric, mean)

#compare species richness
summary(aov(`Species Richness` ~ Grazing, transition_averaged))
TukeyHSD(aov(`Species Richness` ~ Grazing, transition_averaged))
#compare shannon diversity
summary(aov(`Shannon Diversity` ~ Grazing, transition_averaged))
TukeyHSD(aov(`Shannon Diversity` ~ Grazing, transition_averaged))
#compare relative cover of natives
summary(aov(`Relative Cover of Natives` ~ Grazing, transition_averaged))
TukeyHSD(aov(`Relative Cover of Natives` ~ Grazing, transition_averaged))


# Plot inundation over all three seasons ------------------------------------
all_years<-all_data%>% 
...
#species richness
m1<-lmer(spec_rich ~ Grazing*Hoofprint+ Size+ Catchment+Depth+(1|Pool.ID), data=transition_diversity)#fixed-effect model matrix is rank deficient so dropping 1 column / coefficient
m2<-lmer(spec_rich ~ Grazing+ Hoofprint+ Size+ Catchment+Depth+ (1|Pool.ID), data=transition_diversity)#removing interaction terms
anova(m1) #interaction terms didn't really make a difference
anova(m2)
AIC(m1,m2)#m1 is barrrely lower AIC
...
#native cover
m1<-lmer(rel_cov_nat ~ days*Hoofprint+ (1|Pool.ID), data=transition_diversity)#Some predictor variables are on very different scales: consider rescaling 
m2<-lmer(rel_cov_nat ~ days+Hoofprint+ (1|Pool.ID), data=transition_diversity)
anova(m1) #interaction terms didn't really make a difference
anova(m2)
AIC(m1,m2)#m2 is lower AIC
